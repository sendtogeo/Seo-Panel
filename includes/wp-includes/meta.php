...
function add_metadata($meta_type, $object_id, $meta_key, $meta_value, $unique = false) {
    ...
    if (null !== $check)
        return $check;
    if($unique){
        $sp_db->where('meta_key',$meta_key);
        $sp_db->where('related_to_id',$object_id);
        if ($sp_db->getValue ($table,"count(*)") ){
    ...
    //$result = $sp_db->insert($table, array(
    $mid = $sp_db->insert($table, array(
        $column => $object_id,
        'meta_key' => $meta_key,
...
function update_metadata($meta_type, $object_id, $meta_key, $meta_value, $prev_value = '') {
    ...
    $sp_db->where ($column, $object_id);
    $sp_db->where ('meta_key', $meta_key);
    $cols = Array ($id_column);
    $meta_ids = $db->get ($table, null, $cols);
    ...
    if (!empty($prev_value)) {
        ...
        $sp_db->where ('meta_value', $prev_value);
    }
    ...
    $result = $sp_db->update($table, $data);
    if (!$result){
        return false;
    }
...
function delete_metadata($meta_type, $object_id, $meta_key, $meta_value = '', $delete_all = false) {
    ...
    if ('' !== $meta_value && null !== $meta_value && false !== $meta_value){
        ...
        $sp_db->where ('meta_value', $meta_value);
    }
    ...
    $db->where($id_column, $meta_ids, 'IN');
...
function get_metadata($meta_type, $object_id, $meta_key = '', $single = false) {
    ...
    if (isset($res[0])) {
        if ($single && $meta_key){
            ...
            foreach($res as $row){
                if($meta_key){
                    $new_array[] = maybe_unserialize($row["meta_value"]);
                }else{
                    $new_array[] = array("meta_key"=>$row["meta_key"],"meta_value"=>maybe_unserialize($row["meta_value"]));
                }
            }
            return $new_array;
        }
...
function get_metadata_by_mid($meta_type, $meta_id) {
    ...
    //$id_column = ( 'user' == $meta_type ) ? 'umeta_id' : 'meta_id';
    $id_column = 'meta_id';
    $sp_db->where($id_column,$meta_id);

    //$meta = $wpdb->get_row($wpdb->prepare("SELECT * FROM $table WHERE $id_column = %d", $meta_id));
    $meta = $sp_db->getOne($table);
...
function delete_metadata_by_mid($meta_type, $meta_id) {
    ...
    if ($meta = get_metadata_by_mid($meta_type, $meta_id)) {
        ...
        $sp_db->where($id_column,$meta_id);
        $result = (bool) $sp_db->delete($table);
...
function _get_meta_table($type) {
    ...
    $table_name = 'meta_' . $type;


    $res = $sp_db->rawQuery("SHOW TABLES LIKE '" . $table_name . "'");
    if ($sp_db->count > 0) {
        return $table_name;
    } else {
        return FALSE;
